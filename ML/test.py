...
def auc_roc(y_score: np.array, y: np.array):
    ...
    pairs = []
    for eta in np.linspace(0, 1, num=10000, endpoint=True):  # 根据阈值离散化,记录(FPR,TPR)对
        y_pred = (y_score >= eta)
        tpr = TPR(y_pred, y)
        fpr = FPR(y_pred, y)
    ...
    for fpr, tpr in pairs[1:]:  # recall随阈值增加而减小，阈值0对应(1,1), 阈值1对应(0,0)
        area += -(fpr - pre_fpr) * (pre_tpr + tpr) / 2  # fpr为eta的减函数
        pre_fpr, pre_tpr = fpr, tpr  # 下一个小梯形
    return area
...
def kmeans(X, k, T, eps, seed):  # kmeans++, O(Tnkd)
    np.random.seed(seed)
    n, d = X.shape
    centers_idxs = np.random.randint(0, n, k)  # 原始kmeans
    pre_centers, cur_centers = np.zeros((k, d)), X[centers_idxs, :]
    labels = np.zeros(n)
    for t in range(T):
